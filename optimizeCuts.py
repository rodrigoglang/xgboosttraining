...
def runOptimization(workDirectory, config, muonPhase, zenith, azimuth, offset, spectralIndex, normalizationFactor, minimumSignalEfficiency, verbose, environmentVariables):
    ...
    f_off = sorted(glob(workDirectory + '/hap/Optimization-Offruns_' + config + '_' + muonPhase + '_' + str(zenith) + 'deg_' + str(azimuth) + 'deg_events.root'))
    ...
    for fname in f_off:
        ...
        try:
            ...
            sys.exit("ERROR! Problems reading the trees in the gamma files, please check that these variables are present: 'RunNr', 'RaSystem', 'DecSystem', 'RaEvent', 'DecEvent', 'CorrEnergy', " + opt_var + "!")
    ...
    f_mc = sorted(glob(workDirectory + '/hap/Optimization-Gamma_' + config + '_' + muonPhase + '_' + str(zenith) + 'deg_' + str(azimuth) + 'deg_' + offsetString + 'deg_events.root'))
    ...
    thsq_scan_bins = np.linspace(10., 13., 151)
    if 'mono' in config:
