...
from generateLookups import makeLookups_EnergyShape, makeLookups_OffEnergyShape, makeLookups
from generateTrees import generateTrees
from generateTreesForOptimization import generateInputTreesForOptimization
from optimizeCuts import runOptimization
from plotOptimization import plotOptimization, GetOptimizedCuts
import utils
import shutil
...
if finishLookupTables:
    ...
    for config in configs:
        ...
        for muonPhase in muonPhases:
            ...
            utils.wait_for_jobs_to_finish(jobs)
            shutil.move(workDirectory + "/config/" + config + "/result/EffectiveArea_norefit.root",workDirectory + "/config/" + config + "/EffectiveArea_norefit.root")
